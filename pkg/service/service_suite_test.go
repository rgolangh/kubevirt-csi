...
var _ = Describe("NodeService", func() {
	var (
		...
		kubevirtClient = kubevirt.NewMockClient(ctrl)
	)
	BeforeEach(func() {
		underTest = NodeService{
		...
		underTest.deviceLister = func() ([]byte, error) {
			json := fmt.Sprintf("{\"blockdevices\": [{\"serial\":\"%s\", \"path\":\"%s\", \"fstype\":null}]}", serialID, "/dev/sdc")
			return []byte(json), nil
		}
		underTest.dirMaker = func(string, os.FileMode) error {
			return nil
		}
	...
	Describe("Staging a volume", func() {
		Context("With non-matching serial ID", func() {
			It("should fail", func() {
				_, err := underTest.NodeStageVolume(nil, &csi.NodeStageVolumeRequest{
					VolumeId:      "pvc-123",
					VolumeContext: map[string]string{serialContextParameter: "serial000"},
				})
		...
		Context("With Block mode", func() {
			It("should fail", func() {
				_, err := underTest.NodeStageVolume(nil, &csi.NodeStageVolumeRequest{
					...
					VolumeContext: map[string]string{serialContextParameter: serialID},
				})
				Expect(err).To(HaveOccurred())
			})
		})
		...
		Context("With failure to make new filesystem", func() {
			It("should fail", func() {
				underTest.fsMaker = func(device, path string) error {
	...
	Describe("Publishing a volume", func() {
		...
		Context("With failing mkdir", func() {
			It("should fail", func() {
				underTest.dirMaker = func(s string, mode os.FileMode) error {
